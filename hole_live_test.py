#!/usr/bin/env python3
"""Validate v9 live-hole accounting through the public snapshot probe."""
...
def main():
    ...
    if not metadata.startswith("#mini_malloc_hole_v9,"):
        raise AssertionError(f"unexpected metadata: {metadata}")
    if len(rows) < 5:
        raise AssertionError(f"expected at least 5 snapshots, got {len(rows)}")
    # The first allocation may also emit v9's opportunistic initial snapshot,
    ...
    print("v9 live allocation/free/realloc accounting passed")
